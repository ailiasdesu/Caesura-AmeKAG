...
```
用户：aimax-command-plan 我需要在市场结算时添加实时通知功能
...
计划完成后：
- 使用 `aimax-command-tdd` 通过测试驱动开发实施
- 如果出现构建错误，使用 `aimax-command-build-fix`
- 使用 `aimax-command-code-review` 审查已完成的实施

## 相关代理

此命令调用位于以下位置的 `planner` 代理：
`.agents/skills/aimax-agent-planner/SKILL.md`
