...
```
用户：aimax-command-e2e 测试市场搜索和查看流程
...
- 使用 `aimax-command-plan` 识别需要测试的关键流程
- 使用 `aimax-command-tdd` 进行单元测试（更快、更细粒度）
- 使用 `aimax-command-e2e` 进行集成和用户流程测试
- 使用 `aimax-command-code-review` 验证测试质量

## 相关代理

此命令调用位于以下位置的 `e2e-runner` 代理：
`.agents/skills/aimax-agent-e2e-runner/SKILL.md`
