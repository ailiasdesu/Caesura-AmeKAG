...
```
用户：aimax-command-tdd 我需要一个计算市场流动性评分的函数
...
**E2E 测试**（使用 `aimax-command-e2e` 命令）：
- 关键用户流程
- 多步骤流程
- 全栈集成
...
- 首先使用 `aimax-command-plan` 了解要构建什么
- 使用 `aimax-command-tdd` 通过测试来实现
- 如果出现构建错误，使用 `aimax-command-build-fix`
- 使用 `aimax-command-code-review` 审查实现
- 使用 `aimax-command-test-coverage` 验证覆盖率

## 相关代理

此命令调用位于以下位置的 `tdd-guide` 代理：
`.agents/skills/aimax-agent-tdd-guide/SKILL.md`

并可以引用位于以下位置的 `tdd-workflow` 技能：
`.agents/skills/aimax-tdd-workflow/SKILL.md`
