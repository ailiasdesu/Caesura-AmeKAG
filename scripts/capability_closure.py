...
def render_markdown(records, private, declared_total, oos, generated_at, fp):
    ...
    L.append("   绝不静默忽略。")
    L.append("8. **同文件工具函数/委托链内的效果面调用不被 v3 判据捕获**：v3 只扫 handler 直接体，")
    L.append("   handler 调用的同文件 local function（layout/layout_slot 的 apply_container、tween 的")
    L.append("   resolve_layer/step_tween、vibrate 委托的 trans.vib）内部的效果面调用属已核真形态——")
    L.append("   t110/t113 两批 18 命令人工核真中 5 例属此，由人工证据层（overrides）弥补；")
    L.append("   v4 候选=同文件 local function 一层调用穿透。")
    L.append("")
    L.append("## 复现")
    L.append("")
    L.append("```")
