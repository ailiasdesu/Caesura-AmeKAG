...
def _git_evidence_head() -> Optional[str]:
    ...
    red on the very commit that syncs the matrix. Returns None when git is
    missing, ROOT is not a repository, the command fails, or produces no output
    -- callers then fall back to the yaml value.
    """
    try:
        proc = subprocess.run(
...
def resolve_head_commit(cli_head: Optional[str], data: dict) -> Tuple[Optional[str], str]:
    ...
    input data dict is never mutated. When git is unavailable or ROOT is not a
    repository a single [WARN] line goes to stderr and the yaml value is used,
    so a missing git env never falsely reds --check (drift is then skipped).
    """
    if cli_head:
    ...
    print(
        f"[WARN] git evidence HEAD unavailable (git missing or ROOT is not a repository); "
...
def generate_markdown(data: dict, head_commit: Optional[str] = None, head_source: str = "yaml") -> str:
    ...
    output byte-for-byte. head_source ('cli' / 'git' / 'yaml') is recorded in
    the header comment for audit. The input data dict is never mutated.
    """
    ...
    lines.append(f"<!-- head_commit source: {head_source} -->")
    lines.append("")
...
def main():
    ...
    generated_md = generate_markdown(data, effective_head, head_source)
