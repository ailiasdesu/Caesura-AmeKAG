...
> 状态：**scaffold-less reference/未验证**（2026-08-23）。本机无 NDK；
> 本文只做**事实清单与执行路径**，未提供任何无法验证的 gradle/JNI 代码。
> 一切 build/device 结论以 docs/platform/android-device-validation.md 的分层为准。
...
| 平台依赖 | `cmake/CaesuraModules.cmake:70-102` SystemDependencies **无 ANDROID 分支**（R9）；FFmpeg 仅 WIN32（Android 用 pl_mpeg 回退，build_android.sh 强制 CAESURA_ENABLE_FFMPEG=OFF：129-132） |
