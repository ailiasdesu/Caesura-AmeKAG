...
describe('web player resource layout', () => {
  it('serves scripts index, story bundle and demo scenes', () => {
    ...
    if (existsSync(join(root, 'cache', 'story', 'story.lua'))) {
  ...
  it('story bundle lists every demo scene and its assets', () => {
    const bundlePath = join(root, 'cache', 'story', 'story.lua')
    if (!existsSync(bundlePath)) return
    const story = readFileSync(bundlePath, 'utf8')
  ...
  it('dist build layout: runtime dirs copied, bundle under web-assets', () => {
    ...
    if (existsSync(join(dist, 'cache', 'story', 'story.lua'))) {
  ...
  it('dist story bundle still carries the tutorial scenes (round 43/44)', () => {
    const distStory = join(here, 'dist', 'cache', 'story', 'story.lua')
    if (!existsSync(distStory)) return
    const story = readFileSync(distStory, 'utf8')
