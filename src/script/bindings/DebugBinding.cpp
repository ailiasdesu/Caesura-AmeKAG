...
// Helper: get DebugManager or push nil+error and return nullptr
static inline IDebugManager* getDM(lua_State* L) {
    auto* dm = BackendRegistry::instance().getDebugManager();
    if (!dm) {
        lua_pushnil(L);
        lua_pushstring(L, "DebugManager not available");
    }
    return dm;
}
...
static int lua_Debug_get_subsystem_stats(lua_State* L) {
    ...
    else if (s == "audio")     ss = SubSys::Audio;
    else if (s == "scripting") ss = SubSys::Scripting;
    else if (s == "input")     ss = SubSys::Input;
    else if (s == "platform")  ss = SubSys::Platform;
    else if (s == "engine")    ss = SubSys::Engine;
    else {
        lua_pushnil(L);
        lua_pushfstring(L, "Unknown subsystem: %s", name);
        return 2;
    }
    auto* dm = getDM(L);
    if (!dm) return 2;
