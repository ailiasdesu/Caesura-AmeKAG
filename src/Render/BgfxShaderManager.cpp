...
// Internal helper — wraps raw bytecode in bgfx binary header format.
namespace {
...
bool BgfxShaderManager::initEmbeddedShaders() {
    ...
    if (!vsCode || !fsCode) {
        printf("[BgfxShaderManager] No embedded shaders for %s. Debug text only.\n",
               bgfx::getRendererName(renderer));
        return false;
    }
    ...
    if (!bgfx::isValid(m_fallbackProgram)) {
        fprintf(stderr, "[BgfxShaderManager] createProgram rejected. Debug text only.\n");
    }
    ...
    auto createProgramFromDXBC = [&](const uint8_t* vsData, size_t vsSize,
        ...
        const uint16_t attrs[] = { 0x0001, 0x0010 };
        bgfx::ShaderHandle v = buildBgfxShader(vsData, (uint32_t)vsSize, false, 2, attrs);
        bgfx::ShaderHandle f = buildBgfxShader(fsData, (uint32_t)fsSize, true, 0, nullptr);
        ...
        bgfx::ProgramHandle prog = bgfx::createProgram(v, f, true);
