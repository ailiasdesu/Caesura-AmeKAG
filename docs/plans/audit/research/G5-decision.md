...
## 评分分歧调和（A=4.33 > C=3.28 > B=3.03 vs 本文决策 B 首选）

路径 C 报告按「完整引擎复用 35% / 工作量 30% / 风险 20% / 收益 15%」加权给出
A > C > B —— 该视角假设目标是**完整保真移植**（bgfx 渲染/3D/Live2D 全保留），
此时 B 确为净损失。但 B 报告是**实勘级**：tokenizer 仅依赖纯 Lua lpeg、
scheduler 100% 纯 Lua、9 个命令 handler 只调绑定口 —— 执行栈 90%+ 可直接复用，
VN 核心可视子集（30-40 命令）用 DOM/CSS/Canvas/WebAudio 适配即可覆盖主流程，
**成本仅为 A 的 1/4 且不碰 C++**。

**调和结论**：分两个时间窗——
- **近期（G5 实施）**：路径 B 出**可玩 Web demo**（最短杠杆，验证分发缺口）；
- **中期**：路径 A 的 bgfx-wasm 单线程渲染 PoC（若渲染管线闭环，再评估全量移植）；
- **远期**：路径 C 移动端作为第二分发曲线。

两报告事实无冲突，分歧仅在「近期目标定义」：B 报告面向 demo 级分发，
C 报告面向保真级移植。文档决策以近期可交付为准。
