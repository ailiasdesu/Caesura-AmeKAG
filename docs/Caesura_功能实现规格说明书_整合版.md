...
> 版本: 2026-06-07 | 决策: 75项(含Beta Job System) | 状态: Alpha 完成→Beta路线图 | P0: 16/16, P1: 32/32, P2: 8/23 | 构建: Debug+Release 通过
...
**代码架构**: 四层抽象 + 服务定位器 + 统一代理表, 完整解耦 Lua 与 C++ 具体实现。

```
scripts/*.lua
       |
       v
_CAESURA_BACKEND 代理表 (UnifiedBinding)    <- 工厂模式, 单一入口
       |
       v
IRenderDevice / IAudioBackend               <- 抽象接口 (56 纯虚方法)
IPlatformBackend / IAssetProvider
       |
       v
...
```
Lua (scripts/) -> Binding (Scripting/) -> 抽象接口 -> BackendRegistry -> 具体实现
```

- 所有 Binding 文件只 `#include` 抽象接口头文件, 禁止直接依赖 `BgfxRenderDevice`/`SoLoudAudioEngine`
- 替换实现只需改 `Engine::init()` 中 2 行代码, 例如从 SoLoud 切换到 FMOD
- `_CAESURA_BACKEND` 代理表映射 30+ 命令到 `Render`/`KAG`/`DevCore`/`VFX` 全局表, 无重复逻辑

**决策**: 所有 Lua 侧子系统调用必须通过抽象接口 + BackendRegistry 完成, 禁止绕过接口直接访问具体实现。`[10.2.79]`. **P0**.
# Part 1: 脚本引擎
...
> ?? **[代码尚未实现 — Beta 路线图]** 以下 Part 8 为 Beta 阶段目标规格，当前代码库中尚不存在对应的 JobSystem/AssetPipeline/MainThreadJobQueue 实现。
# Part 8: 并行任务系统 (Beta)
...
**决策**: Alpha 完成→Beta路线图 | P0: 16/16, P1: 32/32 | 构建工具管理 CRL, 用于紧急吊销泄露的子密钥。expiry "none" 的永久证书可通过 CRL 吊销, 兼顾长期流传与安全可控。**P0**.
**P2** (CRL 远程更新).
