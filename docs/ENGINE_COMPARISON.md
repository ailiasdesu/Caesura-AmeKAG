...
| Feature | Ren'Py | KAG3 | Tyrano | WebGal |
|---|:---:|:---:|:---:|:---:|
| C++20 + KAG Compatible | No | Closed source | No | No |
| AI-Assisted IDE (F5, @generate, @fix) | No | No | No | No |
| 3D Mini-Games (PBR + Physics) | No | No | No | No |
| AES-256-GCM Save Encryption | No | No | No | No |
| 3-Platform CI (zero errors) | Community only | None | None | Partial |
| 8 Pure Virtual Interfaces | No | No | No | No |
| RPC Editor-Engine Protocol (8 methods) | No | No | No | No |
| Delta CARC + Ed25519 Signing | No | No | No | No |
| 135 Unit Tests + TDR Protection | No | No | No | No |
...
### Choose Caesura
- KAG script compatibility with modern C++ performance
- 3D mini-games inside visual novels
- AI-assisted coding (@generate, @fix)
- Cross-platform from day one (Win/Mac/Linux)
- Full control (MIT open source, no runtime fees)
- Save encryption and asset protection needed

### Choose Ren'Py
- Massive community and thousands of tutorials
- iOS/Android support needed now
- Mature plugin ecosystem (voice, video, etc.)
- Python scripting preferred over Lua/KAG

### Choose KAG3/Kirikiri
- Pure KAG compatibility for legacy Japanese VN ports
- Windows-only is acceptable
- Minimalist engine with no extra features

### Choose TyranoBuilder
- Pure GUI builder -- zero coding required
- Rapid prototyping for non-programmers
- Browser-based distribution only

### Choose WebGal
- Instant browser play with zero install
- Modern TypeScript stack preferred
- Web-native distribution (no desktop app needed)
