...
## 四、定位与差距
- 定位：**KAG Neo-Genesis —— KAG 语法继承之上的新一代标准**（不是 KAG3 的克隆/重构，而是在其基础上更现代的东西：KAG 标签 + Lua 5.4 + bgfx 多后端；保留日系语法免学习，同时**有权抛弃 KAG3 老旧不合理逻辑**——如裸值参数原生支持、参数化宏、有界流程控制、安全白名单）
...
> 全部 CI 三平台验证、review/security_review 覆盖、586/586 测试 + Lua 86/86 全绿。
...
- **定位**：KAG 语法兼容 + **Neo-Genesis 新一代标准** + 活跃维护 + 跨平台 + 免费开源的现代 VN 引擎——
  占据"日系老引擎（停更）→ 现代引擎"迁移空白；KAG3 停更即是机会：旧脚本可迁移，新脚本用现代语义
