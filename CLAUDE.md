...
# Lua script suites (run from repo root; use the build-tree lua_cli product —
# external/lua/lua[.exe] exists only inside release packages):
#   - main suite: 143 files, order-sensitive (sandbox locks globals mid-run)
#   - orphan suite: 26 order-incompatible tests (create global mocks) run
