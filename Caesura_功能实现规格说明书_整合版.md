...
> 版本: 2026-06-07 | 决策: 69项(含Beta Job System) | 状态: Alpha 完成→Beta路线图: P2: 7/67→7项Job System新增 | 构建: Debug+Release 通过
...
- **红区 (Main-Thread-Only):** bgfx 全 API、SoLoud 全 API、lua_State 读写、BackendRegistry 写入。编译期 `[[caesura::main_thread]]` attribute 强制。
...
- 旧 `AsyncLoader` 类标记 `[[deprecated]]`，内部委托给 `AssetPipeline`。详见 Part 8。
...
> ⚠️ **[代码尚未实现 — Beta 路线图]** 以下 Part 8 为 Beta 阶段目标规格，当前代码库中尚不存在对应的 JobSystem/AssetPipeline/MainThreadJobQueue 实现。
# Part 8: 并行任务系统 (Beta)
...
**主线程回调:** worker job 完成后，其 `onComplete` 回调入队到 `MainThreadJobQueue`。主线程每帧 `pollMainThreadJobs()` 按 FIFO 顺序执行红区回调。
...
## [9.1] 调试协议 — lua_sethook 断点/单步 ([10.2.18]
## [9.2] IAssetProvider — 资源虚拟文件系统
...
## [9.3] 设计目标 — AES-256-GCM + Ed25519 + zstd + mmap
## [9.4] 文件布局 — Header(64B) + Content + Index(encrypted) + Signature(96B) + PublicKey(32B)
### 签名版本号 ([10.2.10] — SHA-256(header||version||count||hashes), 防降级攻击
## [9.5] 密钥管理 — private.key(开发者), public.key(CARC 末尾)
...
## [9.8] IAssetProvider 抽象接口 + Provider Chain ([10.2.1]
## [9.9] Lua 字节码预编译
## [9.10] 差分更新 — bsdiff + 原子回滚 ([10.2.16]
## [9.11] 构建集成 — car_pack.exe
...
**决策**: 主线程独占。assert(m_ownerThread==id)。不引入锁。Lua 自然串行。 **Release 构建**: `CAESURA_ASSERT_MAIN_THREAD()` 编译为 `((void)0)` 无开销，Debug 构建 abort。
...
**决策**: Alpha 阶段使用 pool 机制 (`acquireCanvas`/`releaseCanvas` + 延迟销毁队列 `m_deferredDestroy`)，主线程独占，零锁开销。Beta 阶段若引入后台加载线程，按需激活 spinlock (`m_poolSpinLock`)，与 [10.2.55] 统一。防御性设计。
...
**决策**: Lua 256MB+纹理 1GB+对象池。显式 GC: [trans]后 step, [save]后 collect, 300帧 step。
...
**Beta 迁移计划**: 引入 `AssetPipeline` 统一异步加载入口 (`loadTextureAsync(path)`→`JobHandle`)。旧 `AsyncLoader` 标记 `[[deprecated]]` 并内部委托给 AssetPipeline。纹理加载四阶段 pipeline 前三个阶段移至 worker 线程。详见 Part 8。
...
Alpha 现状(46 命令/含 Beta 加速):
**核心**: text, ch, p, l, r, er, ruby, font, bg, fg, cl, image, layopt, position, playbgm, stopbgm, playbgmstop, playse, stopse, playvoice, stopvoice, fadebgm, xfadebgm, setbgmvolume, setsevolume, setvoicevolume, wait, waitbgm, waitsound, trans, move, quake, fade, vfx, video, stopvideo, preload, save, load, listsaves, history, eval, emb, skip, reset, pt
**待实现(1.0)**: link, call, return_, switch/case, waitclick, backlog, config, saveplace, loadplace, mode, label, end_, freeimage, movie, playse3d
...
**决策**: Alpha 阶段 pool 回收机制 (`acquireCanvas`/`releaseCanvas`) 替代自旋锁方案。RTT 操作仅在主线程 (`assert(is_main_thread())`)。spinlock 字段代码注释标记为 Beta 多线程预留，当前不编译锁代码，零开销。**P2**。
...
**决策:** 采用 fiber-based Job System，参考 Destiny 引擎模型。线程数 = hardware_concurrency() - 1，work-stealing 无锁队列，fiber 栈 64KB，三优先级 High/Normal/Low，MainThreadJobQueue 串行化红区回调。

**替代方案被否决:** std::async（无 work-stealing）、保持单线程（无法利用多核）。

### [10.2.73] 三区边界模型 (Beta)

**决策:** 红区(Main-Thread-Only) / 绿区(Thread-Safe CPU) / 黄区(Synchronized Access) 三级线程安全边界。编译期红区函数标记 `[[caesura::main_thread]]`，CI clang-tidy 检查。

### [10.2.74] 资源管线并行化 (Beta)

**决策:** 纹理/音频/CARC 验证的全管线在 Job System 并行执行，仅最后一步 GPU/Audio 提交回主线程。纹理四阶段: 读文件→解密+解压→解码→createTexture。音频: 读文件→波形解码→loadFile。

### [10.2.75] 粒子系统并行化 (Beta)

**决策:** ParticleSystem::update() 的物理模拟在 worker 线程并行，GPU 提交保留主线程。Emitter 按 MAX_PARTICLES/thread_count 均分，无竞争。

### [10.2.76] 字体光栅化并行化 (Beta)

**决策:** 每个 worker 持有独立 FT_Library + FT_Face clone，并行光栅化不同 glyph。主线程 FontRenderer 持有权威 atlas。动态扩容期间暂停 worker。

### [10.2.77] 存档序列化并行化 (Beta)

**决策:** JSON 序列化在 worker 线程，磁盘 I/O 和 Lua GameState 恢复在主线程。Worker 只读 schema version 和 migration chain。

### [10.2.78] KAG Conductor 保持串行 (Beta)

**决策:** KAG 协程调度和 Conductor 编排严格绑定主线程串行，不进入 Job System。演出时间线确定性高于并行吞吐量。[jump]/[call]/[return] 执行与 token 流顺序严格一致。
