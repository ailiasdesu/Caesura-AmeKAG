...
> 最后更新: 2026-06-07 | 决策总数: 68（索引项 71） | 状态: Alpha 就绪（P0: 13/13, P1: 30/30, P2: 15/15 Beta规划, 构建: Debug+Release 通过）
...
Lua 256MB 硬上限（`lua_Alloc` hook 强制执行）。纹理预算采用 6 档自适应策略（详见 [10.2.65]），默认自动检测设备性能定档。对象池复用。显式 GC 点。
...
- `texture_budget`: 纹理预算档位。取值 `"auto"`（默认，引擎自动检测）或 0|1|2|3|4|5 手动指定。档位定义见 [10.2.65]。
...
### FFmpeg/H.264 集成(P1) ([10.2.24] — Beta: -DCAESURA_VIDEO_FFMPEG=ON, H.264/VP9/WebM
...
### RTT 池并发 ([10.2.22] — spinlock + 延迟销毁队列, 防御性设计
...
**Release 构建 debug.* 沙箱**: Release 构建下通过 `_CAESURA_CONFIG.dev_mode` 全局变量控制。非 dev_mode 时 `lockdownScriptEnv()` 将 debug 表替换为只读子集（保留 getinfo/traceback/getlocal/getupvalue/getuservalue/getmetatable，移除 sethook/setlocal/setmetatable 等写入方法）；同时重写 `require` 为仅返回 `package.loaded` 缓存（禁止文件系统搜索），`os.execute/os.remove/os.rename/os.exit` 和 `io.open/io.popen` 全部替换为无操作桩。`[eval]/[emb]` 命令使用 `sandbox.lua` 的白名单 _ENV 沙箱，Dev/Release 双模式自动切换。
...
选择自定义 CARC 容器而非标准 zip 的理由：AES-256-GCM 流式加密 + Ed25519 签名 + CRL 吊销链式信任模型无法用标准容器表达。Alpha 阶段即提供完整安全体系。

## [9.1] 设计目标 — AES-256-GCM + Ed25519 + zstd + mmap
## [9.2] 文件布局 — Header(64B) + Content + Index(encrypted) + Signature(96B) + PublicKey(32B)

**输入验证上限**: 索引条目 ≤ 10,000 文件；metadata JSON ≤ 1MB；header 字段范围验证（magic/version/content_size 合法性检查）。
...
? zstd 压缩块对差分不友好——小逻辑变更可能导致整个压缩块差异巨大。建议评估在差分前解压→bsdiff→重新压缩的流程，或使用 zstd 的 `--patch-from` 模式。
...
| 64 | 移动平台适配 (iOS/Android) | P2 | 平台 |
| 65 | 纹理预算 6 档自适应（新增） | P0 | 系统/性能 |
| 66 | 密码学安全随机数生成（新增） | P0 | CARC/安全 |
| 67 | RTT 帧缓冲生命周期（新增） | P0 | 渲染 |
| 68 | 全局共享 FreeType 实例（新增） | P1 | 字体 |
| 69 | 音频波形 LRU 逐出（新增） | P1 | 音频 |
| 70 | ShaderCache 所有权模型（新增） | P1 | 渲染 |
| 71 | 引擎关机顺序（新增） | P2 | 系统 |
### 优先级分布
...
| 优先级 | 已实现 | 总计 | 缺失项 |
|:------:|:-----:|:----:|------|
| P0 | 12 | 16 | Shader 64KB 上限、[10.2.66]/[10.2.67]（新增，未实现） |
| P1 | 27 | 33 | 线程安全断言、存档跨版本迁移、CARC 自验证、[10.2.68]/[10.2.69]/[10.2.70]（新增，未实现） |
| P2 | 0 | 15 | 全部 Beta 迭代 + [10.2.71]（新增，未实现） |
| Confirmed | 4 | 4 | — |
| **总计** | **39** | **68** | 4 项 Alpha 待解决 + 14 项 Beta 规划 |

> 注: 4 项 Beta 安全特性（CRC32 反调试 [Beta]、HMAC 存档签名 [Beta]、api_seal 混淆 [Beta]、save_bind_to_device 可选设备指纹 [Beta]）仅保留设计，不阻塞 Alpha 编码。新增 [10.2.65] 纹理预算自适应（P0，已纳入决策）。
...
**音频直调与批量执行顺序保证**: 直调用立即生效, 帧末批量按 accumulate 顺序执行。直调先于批量, 禁止同一流混用。**P1** 混用检测: C++ 音频管理器维护 per-stream 状态: `first_op_mode`(None/Batch/Direct, 每帧初重置) + `has_executed_direct`。在 play_xxx 入口处立即检查: 若当前帧 first_op_mode 已锁定且与本次操作模式不同 → 冲突。处理策略: **Debug 模式**: Lua 抛错 `audio_mixed_mode`; **Release 模式**: 自动将所有操作转为 Direct 模式(降级, 保证功能不缺失), 记录一次警告日志。帧末提交后重置所有 per-stream 状态。禁止同一流混用。**P0**。
...
**决策**: Alpha 阶段主线程断言 + 延迟销毁队列; Beta 阶段按需激活 spinlock。防御性设计, 与 [10.2.55] 统一。
...
**决策**: Lua 256MB+纹理 1GB+对象池。显式 GC: [trans]后 step, [save]后 collect, 300帧 step。 所有着色器 `bgfx::UniformHandle` 必须在初始化时创建并缓存复用，禁止在渲染循环内调用 `bgfx::createUniform()`。
...
**队列管理**: 异步加载队列上限 32。队列满时提交者阻塞（生产者-消费者反压），不会静默丢弃请求。

**决策**: createTextureAsync + [preload] 标签 + 过渡槽。后台线程 I/O 后通过 SDL_PushEvent 派发到主线程，确保 bgfx 调用的线程安全。
...
**决策**: kag.ch/text 自动 _T 替换。Backlog 存 key 实时翻译(显示时根据当前语言实时翻译, 历史条目不持久化翻译后文本, 切换语言后 Backlog 内容自动更新)。%{key} 插值。
...
**决策**: Intel HD 620, 2C 2.0GHz, 4GB RAM, 60 FPS。每帧: SDL<0.5, Lua<3, Audio<0.5, Layer<1, GPU<10ms。 所有渲染子系统必须从 BgfxRenderDevice 读取当前后备缓冲区尺寸，禁止硬编码分辨率常量。
...
_ENV whitelist: KAG,Render,math,string,table. Block: os,io,package,debug,require,dofile,loadfile. **Render 仅暴露高层游戏命令**, 完整白名单: load_image, set_blend, set_blend_mode, create_viewport, destroy_viewport, draw_viewport, create_solid_texture, get_viewport_texture, set_viewport_clear, create_texture_async。**禁止直接访问 bgfx 底层 API** (submit, setViewRect, setTransform 等 Render 内部实现细节, 不入 Lua API)。Dev mode relaxed, release strict. **P0**.
...
--- [Retry] 同一 token >3 次自动 Title。[Retry] 重新初始化命令全部状态, 新建 CancelToken。**Title 崩溃保护**: 全局错误计数器, 若连续 Title 回退 3 次仍崩溃, fallback 到 SDL_ShowSimpleMessageBox(仅显示错误文本 + 退出按钮), 不再尝试恢复脚本。**P1**。
...
**决策**: 缩略图编码循环: (1) 以 config.thumbnail_quality(默认 80) 编码 (2) 若 PNG 大小 > config.thumbnail_max_kb(默认 64KB): 分辨率折半(320->160->80) (3) 再次超过限制: 保存纯色占位缩略图(1x1 灰色像素), 标记 truncated=true。流程: 主线程 readTexture -> 后台 PNG 编码 -> 回调更新 JSON。readTexture >2ms 则跳过。**P2**.
...
---
**GraphicsMonitor**: 每帧 gpuTimeMs。连续 3 帧 >18ms 降级, 30 帧 <12ms 恢复。降级顺序: (1) 关闭抗锯齿 (2) 降低 RTT 分辨率到 0.5x (3) 禁用后处理特效 (4) 降级为内置字体。逐级尝试, 每级等待 10 帧评估效果。config.quality 允许用户手动设置档位(low/medium/high/auto), 禁用自动降级。
...
**决策**: 纹理预算由引擎根据设备性能自动检测定档，开发者可通过 config.lua 中 	exture_budget 配置项覆盖。

| 档位 | 纹理预算 | 触发条件（自动检测） | 适用硬件 |
|:----:|---------|---------------------|---------|
| 0 | 128 MB | 总 RAM < 4GB 或 GPU 共享内存 | 低端集显 |
| 1 | 256 MB | 总 RAM 4-6GB，集显 | Intel HD 620 默认 |
| 2 | 512 MB | 总 RAM 6-8GB，独显 < 2GB VRAM | 入门独显 |
| 3 | 1 GB | 总 RAM 8-16GB，独显 2-4GB VRAM | 主流独显 |
| 4 | 2 GB | 总 RAM > 16GB，独显 > 4GB VRAM | 高端独显 |
| 5 | 4 GB | 开发者强制覆盖（	exture_budget = 5） | 开发/测试 |

**配置接口**（[4.3]）: 	exture_budget = "auto"（默认）| 0|1|2|3|4|5。设为 "auto" 时引擎通过 SDL_GetSystemRAM() + GPU capability 查询自动定档；显式设档位值则跳过自动检测。

**架构约束**: 纹理预算在引擎初始化时确定，运行时不可切换档位（避免纹理重分配开销）。
...
**决策**: 引擎信任主 CARC Ed25519 公钥。子 CARC 使用独立密钥, 其公钥由主公钥签发授权证书(含子公钥哈希+有效期+权限范围), **证书存储于主 CARC chain/ 目录**。验证: (1) 读子公钥(子CARC末尾32B) (2) 读主CARC chain/下对应证书 (3) 主公钥验证证书签名 (4) 比对证书内公钥哈希与末尾公钥 (5) 子公钥验签。**P0**。
...
**决策**: AES-256 密钥、GCM 96-bit nonce、Ed25519 种子必须使用 `BCryptGenRandom()`（Windows）或 `/dev/urandom`（Linux/macOS）生成。禁止 `std::mt19937`、`rand()`、`srand()` 及任何非密码学安全 PRNG。**P0**。

### [10.2.67] RTT 帧缓冲生命周期

**决策**: `flushAllRTT()` 必须在 `bgfx::shutdown()` 之前遍历所有活跃 RTT 并调用 `bgfx::destroy(fb)`。顺序: flushAllRTT() → bgfx::shutdown()。禁止依赖 bgfx 隐式清理。**P0**。

### [10.2.68] 全局共享 FreeType 实例

**决策**: TextRenderer 和 FontRenderer 共享单一 `FT_Library` 实例和字体图集。禁止各自独立初始化 FreeType（节省 ~8MB 重复图集内存）。**P1**。

### [10.2.69] 音频波形 LRU 逐出

**决策**: SoLoud 波形缓存使用 LRU 逐出策略（`std::list` + `std::unordered_map`），禁止超限时全量清除（全量清除导致可听卡顿）。**P1**。

### [10.2.70] ShaderCache 所有权模型

**决策**: CompositeShaderCache 仅索引 `bgfx::ProgramHandle`，不拥有所有权。BgfxRenderDevice 负责创建和销毁。禁止 ShaderCache::shutdown() 调用 `bgfx::destroy()`（防止二次销毁）。**P1**。

### [10.2.71] 引擎关机顺序

**决策**: Engine::shutdown() 顺序: AsyncLoader → VideoPlayer → Lua → Audio → Render → Platform。依赖方先于被依赖方销毁（Lua 在 VideoPlayer 前关机可避免脚本引用悬空视频句柄）。**P2**。
**Alpha 剩余工作**: 1 项 P0（Shader 64KB 上限）+ 3 项 P1（线程安全断言、存档跨版本迁移、CARC 自验证）。4 项 Beta 安全特性（CRC32 反调试、HMAC 存档签名、api_seal `[Beta]`（代码混淆层计划 Beta 阶段加入） 混淆、`save_bind_to_device` 可选设备指纹（默认关闭，Beta）可选绑定）在 Alpha 阶段仅保留设计，不阻塞编码。


## [10.3] 实施优先级

### P0（16 项）: 编码前规格解决
1.[10.2.31]GameState 2.[10.2.33]CancelToken 3.[10.2.5]JSON 4.[10.2.47]沙箱 5.[10.2.51]沙箱开关 6.[10.2.1]Provider 7.[10.2.28]公钥外置 8.[10.2.63]多CARC公钥 9.[10.2.10]签名版本 10.[10.2.7]Layout 11.[10.2.11]混合模型 12.[10.2.2]PTS 13.[10.2.3]3D安全 14.[10.2.65]纹理预算 15.[10.2.66]密码学PRNG 16.[10.2.67]RTT生命周期
...
| 维度 | 状态 |
|------|------|
| 安全模型 | Alpha 基础 / Beta 完整（CRC32/HMAC/api_seal 为 Beta 路线图） |
...
**结论**: 68 项设计中新增 6 项（[10.2.66]-[10.2.71]），累计 57 项已实现（93%）。P0: 12/16, P1: 27/33, P2: 0/15 规划中, Confirmed: 4。Alpha 编码进行中，4 项 Beta 安全特性仅保留设计不阻塞编码。
...
**注**: 28 种混合模式为 W3C Compositing Level 1 全量实现，为引擎创意表达能力提供完整画布。核心 VN 路径常用约 6 种（Normal/Multiply/Screen/Overlay/Add/Alpha），其余模式按需激活，不影响着色器体积（单个 `blend.glsl` 中 `switch` 分支）。? 最坏场景（5+ 层不同混合模式同时活跃）在 Intel HD 620 上的 GPU 帧时间待 Alpha 实测验证。
