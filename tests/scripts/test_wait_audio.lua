...
-- (the module's `local backend` captured the mock at load)
...
local f = assert(io.open("scripts/kag/commands/audio.lua", "rb"))
local src = f:read("*a")
f:close()
-- gbk decode for the assertion
local okDecode, srcText = pcall(function() return src:decode('gbk') end)
if not okDecode then srcText = src end
check("cap constant", srcText:find("WAIT_AUDIO_LIMIT_MS = 60000", 1, true) ~= nil)
check("waitsound bounded", srcText:find('audio_is_playing("se") and elapsed < WAIT_AUDIO_LIMIT_MS', 1, true) ~= nil)
check("waitbgm bounded", srcText:find('audio_is_playing("bgm") and elapsed < WAIT_AUDIO_LIMIT_MS', 1, true) ~= nil)
