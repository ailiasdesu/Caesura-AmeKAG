-- Caesura (AmeKAG) — First-VN E2E Entry Point (kag_runner + full render loop)
...
local demo_path = nil
for _, p in ipairs({"first_vn/story.ks", "demo/first_vn/story.ks", "tests/projects/first_vn/story.ks", "../tests/projects/first_vn/story.ks"}) do
    if file_exists(p) then demo_path = p; break end
end

if not demo_path then
    print("[FirstVN] FATAL: Cannot find first_vn/story.ks")
    return
end

print("[FirstVN] Loading: " .. demo_path)
local started = kag_runner.start(demo_path)
if not started then
    print("[FirstVN] FATAL: Failed to start story")
    return
end
